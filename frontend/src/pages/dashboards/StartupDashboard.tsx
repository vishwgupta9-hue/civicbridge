...
export const StartupDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"opportunities" | "concepts" | "pilots" | "support">("opportunities");
  ...
  const [proposeTarget, setProposeTarget] = useState<RecommendedOpportunity | null>(null);
  ...
  // Handle Propose Solution Submission
  const handleOpenProposeModal = (opp: RecommendedOpportunity) => {
    setProposeTarget(opp);
    setProposeForm({
      solutionName: "",
      solutionSummary: "",
      productTechOffered: "",
      ...
      estimatedCost: "",
      expectedCivicImpact: `Measurable relief for ${opp.affectedCount.toLocaleString()} citizens in ${opp.district}`,
    });
  ...
  const handleProposeSubmit = async (e: React.FormEvent) => {
    ...
    setProposeError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/startup/propose-solution`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          problemId: proposeTarget.id,
          solutionName: proposeForm.solutionName,
          solutionSummary: proposeForm.solutionSummary,
          productTechOffered: proposeForm.productTechOffered,
          howItSolves: proposeForm.howItSolves,
          deploymentRequirements: proposeForm.deploymentRequirements,
          expectedTimeline: proposeForm.expectedTimeline,
          estimatedCost: proposeForm.estimatedCost || undefined,
          expectedCivicImpact: proposeForm.expectedCivicImpact,
        }),
      });

      const data = await res.json();
      if (!res.ok || !data.success) {
        throw new Error(data.error || (data.details ? JSON.stringify(data.details) : "Failed to submit proposal."));
      }

      setProposeSuccess("Solution proposal submitted successfully! Your solution is now active under My Solutions.");
      setTimeout(() => {
      ...
      setProposeError(err.message || "Failed to submit solution proposal.");
    } finally {
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="inline-block px-2 py-0.5 text-xs font-semibold bg-cyan-100 text-cyan-900 rounded-full">
                  Civic Solution Builder Portal
                </span>
              </div>
              <p className="text-[11px] text-slate-500 hidden sm:block">
                Discover Problems · Deploy Solutions · Run Pilots · Create Civic Impact
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-4 sm:p-6 space-y-6">
          ...
          <>
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start sm:items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-500 to-teal-600 flex items-center justify-center shrink-0 shadow-md">
                    <Rocket className="w-8 h-8 text-white" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h2 className="text-lg font-bold text-slate-900">
                        {dashboardData.organization?.name || user?.organization?.name || "Civic Startup"}
                      </h2>
                    ...
                    <p className="text-xs text-slate-600 mt-0.5">
                      Founder / Lead: <strong className="text-slate-800">{user?.name}</strong>
                    </p>
                    <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                      ...
                      <span>•</span>
                      <span className="font-medium text-cyan-700 flex items-center gap-1">
                        <Rocket className="w-3 h-3" />Civic Solution Builder
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="px-3 py-1.5 bg-cyan-50 text-cyan-800 border border-cyan-200 text-xs font-bold rounded-xl flex items-center gap-1.5">
                    <CheckCircle2 className="w-3.5 h-3.5 text-cyan-600" />STARTUP ROLE
                  </span>
                  <Link
                    to="/problems"
                    className="px-3.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold rounded-xl transition-colors flex items-center gap-1.5 shadow-sm"
                  >
                    <span>Explore Civic Opportunities</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
              </div>

              {/* Hero: What We Build + Technology + Deployment */}
              <div className="pt-3 border-t border-slate-100 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                  <div className="p-3 bg-cyan-50 border border-cyan-100 rounded-xl">
                    <span className="text-[10px] font-bold text-cyan-600 uppercase tracking-wider block mb-1">What We Build</span>
                    <p className="text-slate-800 font-semibold leading-snug">
                      {dashboardData.organization.domainTags?.slice(0, 2).join(" · ") || "Civic Technology Solutions"}
                    </p>
                  </div>
                  <div className="p-3 bg-teal-50 border border-teal-100 rounded-xl">
                    <span className="text-[10px] font-bold text-teal-600 uppercase tracking-wider block mb-1">Technology / Domain</span>
                    <div className="flex flex-wrap gap-1">
                      {dashboardData.organization.expertiseTags?.slice(0, 3).map((tag) => (
                        <span key={tag} className="px-1.5 py-0.5 bg-teal-100 text-teal-800 rounded text-[10px] font-medium">{tag}</span>
                      )) || <span className="text-slate-600">—</span>}
                    </div>
                  </div>
                  <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl">
                    <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider block mb-1">Deployment Capability</span>
                    <p className="text-slate-800 font-semibold leading-snug">
                      {dashboardData.stats.activePilots > 0
                        ? `${dashboardData.stats.activePilots} active pilot${dashboardData.stats.activePilots > 1 ? "s" : ""} in field`
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-1">
                <span className="text-xs font-semibold text-cyan-600 uppercase tracking-wider block">Civic Opportunities</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-cyan-900">{dashboardData.stats.totalClaims}</span>
                  <span className="text-xs text-cyan-600 font-medium">claimed</span>
                </div>
                <p className="text-[11px] text-slate-500">Problems engaged</p>
              </div>
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-1">
                <span className="text-xs font-semibold text-blue-600 uppercase tracking-wider block">My Solutions</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-blue-900">{dashboardData.stats.totalConcepts}</span>
                  <span className="text-xs text-blue-600 font-medium">developed</span>
                </div>
                <p className="text-[11px] text-slate-500">Civic products/services</p>
              </div>
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-1">
                <span className="text-xs font-semibold text-emerald-600 uppercase tracking-wider block">Active Pilots</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-emerald-900">{dashboardData.stats.activePilots}</span>
                  <span className="text-xs text-emerald-600 font-medium">in field</span>
                </div>
                <p className="text-[11px] text-slate-500">Deployed solutions</p>
              </div>
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-1">
                <span className="text-xs font-semibold text-amber-600 uppercase tracking-wider block">Support Requests</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-amber-900">{dashboardData.stats.supportRequestsCount}</span>
            ...
            <div className="flex items-center gap-2 border-b border-slate-200 pb-2 overflow-x-auto">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all min-h-[44px] shrink-0 ${
                  activeTab === "opportunities" ? "bg-cyan-600 text-white shadow-sm" : "bg-white text-slate-700 hover:bg-slate-100 border border-slate-200"
                }`}
              >
                <Sparkles className="w-4 h-4" />
                <span>Civic Opportunities</span>
                <span className={`ml-1 px-2 py-0.5 rounded-full text-[10px] ${activeTab === "opportunities" ? "bg-cyan-700 text-white" : "bg-slate-100 text-slate-700"}`}>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all min-h-[44px] shrink-0 ${
                  activeTab === "concepts" ? "bg-cyan-600 text-white shadow-sm" : "bg-white text-slate-700 hover:bg-slate-100 border border-slate-200"
                }`}
              >
                <TrendingUp className="w-4 h-4" />
                <span>My Solutions</span>
                <span className={`ml-1 px-2 py-0.5 rounded-full text-[10px] ${activeTab === "concepts" ? "bg-cyan-700 text-white" : "bg-slate-100 text-slate-700"}`}>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all min-h-[44px] shrink-0 ${
                  activeTab === "pilots" ? "bg-cyan-600 text-white shadow-sm" : "bg-white text-slate-700 hover:bg-slate-100 border border-slate-200"
                }`}
              >
                <Target className="w-4 h-4" />
                <span>Active Pilots</span>
                <span className={`ml-1 px-2 py-0.5 rounded-full text-[10px] ${activeTab === "pilots" ? "bg-cyan-700 text-white" : "bg-slate-100 text-slate-700"}`}>
                  {dashboardData.myConcepts.filter(c => c.currentStage === "BUILDING" || c.currentStage === "PILOTED").length}
                </span>
              </button>
              <button
                onClick={() => setActiveTab("support")}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all min-h-[44px] shrink-0 ${
                  activeTab === "support" ? "bg-cyan-600 text-white shadow-sm" : "bg-white text-slate-700 hover:bg-slate-100 border border-slate-200"
                }`}
              >
                <HelpCircle className="w-4 h-4" />
                <span>Support</span>
                <span className={`ml-1 px-2 py-0.5 rounded-full text-[10px] ${activeTab === "support" ? "bg-cyan-700 text-white" : "bg-slate-100 text-slate-700"}`}>
                  {dashboardData.supportRequests.length}
                </span>
              </button>
            </div>

            {/* ============================================================= */}
            {/* TAB 1: CIVIC OPPORTUNITIES                                     */}
            {/* ============================================================= */}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                      ...
                      Civic Opportunities for Your Solutions
                    </h3>
                    <p className="text-xs text-slate-500">
                      Problems matched to your domain capabilities, technology focus, and district presence.
                    </p>
                  </div>
                  <Link
                    to="/problems"
                  ...
                  >
                    <span>Browse all statewide problems</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
                      ...
                      >
                        ...
                        <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            {opp.isClaimed ? (
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-600 hover:text-cyan-700 bg-slate-50 hover:bg-cyan-50 border border-slate-200 hover:border-cyan-200 px-2.5 py-1.5 rounded-xl transition-colors min-h-[44px] sm:min-h-0 items-center"
                                title="Claim without immediate proposal"
                              >
                          ...
                          <div className="flex items-center gap-2">
                            <Link
                              to={`/problems/${opp.id}`}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition-colors min-h-[44px] sm:min-h-0 items-center"
                            >
                            ...
                            <button
                              onClick={() => handleOpenProposeModal(opp)}
                              className="inline-flex items-center gap-1.5 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold rounded-xl transition-colors min-h-[44px] sm:min-h-0 items-center shadow-sm shadow-cyan-600/20"
                            >
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Products, services and concepts in development — from initial idea to field deployment.
                    </p>
                  </div>
                </div>
                ...
                {dashboardData.myConcepts.length === 0 ? (
                  <div className="bg-white rounded-2xl p-10 border border-dashed border-slate-300 text-center space-y-3">
                    ...
                    <p className="text-sm font-semibold text-slate-700">No solutions developed yet.</p>
                    <p className="text-xs text-slate-500 max-w-sm mx-auto">Claim a civic opportunity from the Problem Bank and develop your first deployable solution.</p>
                    <button
                      onClick={() => setActiveTab("opportunities")}
                      className="inline-flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold rounded-xl transition-colors"
                    >
                      <Sparkles className="w-4 h-4" />
                      <span>See Civic Opportunities</span>
                    </button>
                  </div>
                ) : (
                  <div className="space-y-4">
            ...
            {/* ============================================================= */}
            {activeTab === "pilots" && (() => {
              const pilots = dashboardData.myConcepts.filter(
                (c) => c.currentStage === "BUILDING" || c.currentStage === "PILOTED"
              );
              return (
                <div className="space-y-4">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                      ...
                      Active Pilots & Deployed Solutions
                    </h3>
                    <p className="text-xs text-slate-500">
                      Solutions currently in build or field deployment phase.
                    </p>
                  </div>
                  {pilots.length === 0 ? (
                    <div className="bg-white rounded-2xl p-10 border border-dashed border-slate-300 text-center space-y-3">
                      <Target className="w-8 h-8 text-slate-400 mx-auto" />
                      <p className="text-sm font-semibold text-slate-700">No active pilots yet.</p>
                      <p className="text-xs text-slate-500 max-w-sm mx-auto">
                        Solutions in the BUILDING or PILOTED stage will appear here. Advance a claimed opportunity to start your first pilot.
                      </p>
                      <button
                        onClick={() => setActiveTab("concepts")}
                        className="inline-flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold rounded-xl transition-colors"
                      >
                        <TrendingUp className="w-4 h-4" />
                        <span>View My Solutions</span>
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {pilots.map((concept) => (
                        <div key={concept.id} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm hover:border-cyan-300 hover:shadow-md transition-all space-y-3">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                            <div className="space-y-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${getStageBadgeClass(concept.currentStage)}`}>
                                  {concept.currentStage.replace(/_/g, " ")}
                                </span>
                                {concept.problem?.district && (
                                  <span className="text-xs font-medium text-slate-500 flex items-center gap-1">
                                    <MapPin className="w-3 h-3" />{concept.problem.district}
                                  </span>
                                )}
                                {concept.problem?.category && (
                                  <span className="text-xs font-medium text-slate-500">• {concept.problem.category}</span>
                                )}
                              </div>
                              <h4 className="text-base font-bold text-slate-900">
                                {concept.title || concept.solutionDescription.slice(0, 60)}
                              ...
                              {concept.problem && (
                                <p className="text-xs text-cyan-700 font-medium">
                                  Civic Problem:{" "}
                                  <Link to={`/problems/${concept.problemId}`} className="underline hover:text-cyan-900">
                                    {concept.problem.title}
                                  </Link>
                                </p>
                              )}
                            ...
                            <Link
                              to={`/problems/${concept.problemId}`}
                              className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold rounded-xl transition-colors self-start shadow-sm"
                            >
                          ...
                          <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-xs">
                            ...
                            <p className="text-slate-600 leading-relaxed line-clamp-2">{concept.solutionDescription}</p>
                          </div>
                          {concept.problem && (
                            <div className="grid grid-cols-3 gap-2 text-xs">
                              <div className="bg-rose-50 p-2.5 rounded-xl border border-rose-100">
                                <span className="text-rose-500 block text-[10px] font-bold uppercase mb-0.5">Priority</span>
                                <strong className="text-rose-900">{concept.problem.priorityTier}</strong>
                              </div>
                              <div className="bg-blue-50 p-2.5 rounded-xl border border-blue-100">
                                <span className="text-blue-500 block text-[10px] font-bold uppercase mb-0.5">Affected</span>
                                <strong className="text-blue-900">{concept.problem.affectedCount.toLocaleString()}</strong>
                              </div>
                              <div className="bg-emerald-50 p-2.5 rounded-xl border border-emerald-100">
                                <span className="text-emerald-500 block text-[10px] font-bold uppercase mb-0.5">Score</span>
                                <strong className="text-emerald-900">{Math.round(concept.problem.priorityScore)}</strong>
                              </div>
                            </div>
                          )}
                          {concept.progressUpdates && concept.progressUpdates.length > 0 && (
                            <div className="pt-2 border-t border-slate-100">
                              <span className="text-[11px] font-bold text-slate-700 mb-1.5 block">Latest Update:</span>
                              <div className="p-2.5 bg-cyan-50/60 rounded-lg border border-cyan-100 text-xs">
                                {concept.progressUpdates[0].milestoneTitle && (
                                  <span className="font-bold text-cyan-900 block">{concept.progressUpdates[0].milestoneTitle}</span>
                                )}
                                <p className="text-slate-700 mt-0.5">{concept.progressUpdates[0].updateText}</p>
                                <span className="text-[10px] text-slate-400">{new Date(concept.progressUpdates[0].createdAt).toLocaleDateString()}</span>
                              </div>
                            </div>
                          )}
                        </div>
        ...
        {proposeTarget && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-2xl max-w-2xl w-full p-6 sm:p-7 space-y-5 shadow-2xl border border-slate-200 my-8 max-h-[90vh] flex flex-col">
              {/* Header */}
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-slate-900">
                      Propose Civic Solution
                    </h3>
                    <p className="text-xs text-slate-500">
                      Submit a concrete technology product or service to solve this problem.
                    </p>
                  </div>
                </div>
                <button
              ...
              {/* Pre-linked Problem Context Box */}
              <div className="p-3.5 bg-gradient-to-r from-cyan-50/80 to-slate-50 rounded-xl border border-cyan-100 text-xs text-cyan-950 shrink-0 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-900 text-xs truncate max-w-md">{proposeTarget.title}</span>
                  <span className="text-[10px] font-semibold px-2 py-0.5 bg-cyan-100/70 text-cyan-800 rounded-md">
                    {proposeTarget.district} · {proposeTarget.category}
                  </span>
                </div>
                <p className="text-slate-600 text-[11px] line-clamp-2">{proposeTarget.description}</p>
                <div className="text-[10px] text-cyan-800 font-medium pt-1 flex items-center gap-1.5">
                  <Users className="w-3 h-3 text-cyan-600" />
                  <span>{proposeTarget.affectedCount.toLocaleString()} citizens affected in target location</span>
                </div>
              </div>
              ...
              {proposeSuccess ? (
                <div className="p-6 bg-emerald-50 text-emerald-900 rounded-xl text-center space-y-2 border border-emerald-200">
                  ...
                  <h4 className="text-sm font-bold text-emerald-900">Solution Proposal Submitted!</h4>
                  <p className="text-xs text-emerald-800">{proposeSuccess}</p>
                </div>
              ) : (
                <form onSubmit={handleProposeSubmit} className="space-y-4 text-xs overflow-y-auto pr-1">
                  {proposeError && (
                  ...
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      Solution Name <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={proposeForm.solutionName}
                      onChange={(e) => setProposeForm({ ...proposeForm, solutionName: e.target.value })}
                      placeholder="e.g. JalShuddhi Solar-Powered IoT Water Filtration Unit"
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                      required
                    />
                  </div>
                  ...
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      What does your solution do? <span className="text-rose-500">*</span>
                    </label>
                    <textarea
                      value={proposeForm.solutionSummary}
                      onChange={(e) => setProposeForm({ ...proposeForm, solutionSummary: e.target.value })}
                      placeholder="High-level overview of the solution, core functionality, and how it operates in the field..."
                      rows={2}
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                      required
                    />
                  ...
                  {/* 3. Product / technology offered */}
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      Product / Technology Offered <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={proposeForm.productTechOffered}
                      onChange={(e) => setProposeForm({ ...proposeForm, productTechOffered: e.target.value })}
                      placeholder="e.g. Multi-stage filtration cartridge, GSM telemetry, edge AI turbidity sensor"
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                      required
                    />
                  </div>

                  {/* 4. How it solves this problem */}
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      How it solves this problem <span className="text-rose-500">*</span>
                    </label>
                    <textarea
                      value={proposeForm.howItSolves}
                      onChange={(e) => setProposeForm({ ...proposeForm, howItSolves: e.target.value })}
                      placeholder="Explain the mechanism of action, direct mitigation of the civic root cause, and end-user benefits..."
                      rows={3}
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                      required
                    />
                  ...
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      Deployment Requirements <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={proposeForm.deploymentRequirements}
                      onChange={(e) => setProposeForm({ ...proposeForm, deploymentRequirements: e.target.value })}
                      placeholder="e.g. 10 sq ft ground footprint, single-phase power (or solar), local ward admin NOC"
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                      required
                    />
                  </div>

                  {/* 6. Expected deployment timeline & Estimated cost */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="font-bold text-slate-800 block mb-1">
                        Expected Deployment Timeline <span className="text-rose-500">*</span>
                      </label>
                      <input
                        type="text"
                        value={proposeForm.expectedTimeline}
                        onChange={(e) => setProposeForm({ ...proposeForm, expectedTimeline: e.target.value })}
                        placeholder="e.g. 4-6 Weeks or 3 Months"
                        className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                        required
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={proposeForm.estimatedCost}
                        onChange={(e) => setProposeForm({ ...proposeForm, estimatedCost: e.target.value })}
                        placeholder="e.g. ₹1,20,000 unit cost / Subscription / CSR-funded"
                        className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                      />
                    </div>
                  </div>

                  {/* 7. Expected civic impact */}
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      Expected Civic Impact <span className="text-rose-500">*</span>
                    </label>
                    <textarea
                      value={proposeForm.expectedCivicImpact}
                      onChange={(e) => setProposeForm({ ...proposeForm, expectedCivicImpact: e.target.value })}
                      placeholder="e.g. 100% reduction in waterborne contaminants for 1,500 residents; 24/7 continuous water telemetry."
                      rows={2}
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:bg-white focus:outline-none text-xs"
                      required
                    />
                  ...
                  <div className="flex justify-end gap-2 pt-3 border-t border-slate-100 shrink-0">
                    ...
                    <button
                      ...
                      className="px-5 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-bold rounded-xl shadow-sm disabled:opacity-50 flex items-center gap-2 transition-colors"
                    >
                      {proposeLoading ? (
                        <>
