...
import { OfferCategory, NeedUrgency } from "../../types";
...
export const CreateNeedModal: React.FC<CreateNeedModalProps> = ({
  ...
  const [urgency, setUrgency] = useState<NeedUrgency>("STANDARD");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "Failed to post resource need.");
      }

      onSuccess();
      onClose();
    } catch (err: any) {
      setError(err.message || "An error occurred creating the need.");
    } finally {
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in fade-in duration-150">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">Post Resource Need</h3>
              <span className="text-[11px] text-slate-500">
                {projectTitle ? `Linking to Project: ${projectTitle}` : "Statewide Institutional Resource Exchange"}
              </span>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div>
            <label className="font-bold text-slate-700 block mb-1">Need Title</label>
            <input
              type="text"
              required
              minLength={3}
              ...
              placeholder="e.g. Need ICP-MS Spectrophotometer for Heavy Metal Testing"
              className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-amber-500 focus:outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="font-bold text-slate-700 block mb-1">District Focus</label>
              <select
                value={district}
            ...
            <div>
              ...
              >
                ...
                <option value="CRITICAL_PATH">Critical Path (Blocking)</option>
              </select>
            </div>
          </div>

          <div>
            <label className="font-bold text-slate-700 block mb-1">Detailed Requirements & Specifications</label>
            <textarea
              required
              minLength={10}
              rows={3}
              ...
              placeholder="Detail required capacity, dates, specs, or required certifications..."
              className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-amber-500 focus:outline-none"
            />
          </div>
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
            ...
            <button
              ...
              className="px-5 py-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-bold rounded-lg shadow-xs transition-colors flex items-center gap-1.5"
            >
              <Plus className="w-4 h-4" />
              <span>{isSubmitting ? "Posting..." : "Publish Need"}</span>
            </button>
          </div>
        </form>
      </div>
